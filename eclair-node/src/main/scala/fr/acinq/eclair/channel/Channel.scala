...
class Channel(nodeParams: NodeParams, remoteNodeId: PublicKey, blockchain: ActorRef, router: ActorRef, relayer: ActorRef)(implicit ec: ExecutionContext = ExecutionContext.Implicits.global) extends LoggingFSM[State, Data] {
  ...
  when(OFFLINE) {
    case Event(INPUT_RECONNECTED(r), d: HasCommitments) =>
      forwarder ! r
      d.resendOnReconnection.map(msg => forwarder ! msg)
      d match {
  ...
  onTransition {
    case state -> nextState =>
      ...
      val messages = (stateData, nextStateData) match {
        case (_, d: DATA_WAIT_FOR_OPEN_CHANNEL) => Nil
        case (_, d: DATA_WAIT_FOR_ACCEPT_CHANNEL) => forwarder ! d.lastSent
        case (_, d: DATA_WAIT_FOR_FUNDING_CREATED) => forwarder ! d.lastSent
        case (_, d: DATA_WAIT_FOR_FUNDING_SIGNED) => forwarder ! d.lastSent
        case (_, d: DATA_WAIT_FOR_FUNDING_CONFIRMED) => forwarder ! StoreAndForward(d.lastSent.right.toSeq, d)
        case (_, d: DATA_WAIT_FOR_FUNDING_LOCKED) => forwarder ! StoreAndForward(d.lastSent :: Nil, d)
        case (_, d: DATA_WAIT_FOR_ANN_SIGNATURES) => forwarder ! StoreAndForward(d.lastSent :: Nil, d)
        case (from: HasCommitments, to: HasCommitments) =>
          ...
          val messages = nextMessages diff currentMessages
          forwarder ! StoreAndForward(messages, to)
        case _ => ()
      }
  }
